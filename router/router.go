...
func Load(g *gin.Engine, mw ...gin.HandlerFunc) *gin.Engine {
	...
	api := g.Group("/api")
	{
		...
		tagGroup := api.Group("/tag")
		{
			...
			tagGroup.GET("/user/detail/:userId/:tagId", tag.QueryUserTagInfo)
			tagGroup.DELETE("/:tagId", middleware.LoginRequired, middleware.IsAdmin, tag.DeleteTag)
			tagGroup.GET("/article/:tagId", tag.QueryTagArticlePagination)
			tagGroup.GET("/detail/:tagId", tag.QueryTagInfo)
